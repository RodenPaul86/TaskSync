...
struct CalendarView: View {
    ...
    var body: some View {
        ...
        .scrollTargetBehavior(CustomScrollBehaveour(maxHeight: maxHeight))
    }
    
    // Test Card View (For Scrolling Content)
    @ViewBuilder
    ...
    // Calendar View
    @ViewBuilder
    func mainCalendar() -> some View {
        GeometryReader {
            let size = $0.size
            let minY = $0.frame(in: .scrollView(axis: .vertical)).minY
            // Convrting Scall into Progress
            ...
            .background(.red.gradient) /// <-- TODO: Change color of calendar
            // Sticking to to the top
            .clipped()
            .contentShape(.rect)
            .offset(y: -minY)
        }
...
#Preview {
    CalendarView(safeArea: EdgeInsets())
}

// Custom Scroll Behaviour
struct CustomScrollBehaveour: ScrollTargetBehavior {
...
struct Day: Identifiable {
    ...
    var date: Date
    var ignored: Bool = false /// <-- Previous/Next Month Excess Dates
}
